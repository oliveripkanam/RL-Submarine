...
EPSILON_END = 0.01
EPSILON_DECAY = 0.99995  # Matching new decay math
TARGET_UPDATE = 1000
SAVE_INTERVAL = 50
...
def train():
    ...
    total_steps = 0
    print(f"Starting Double DQN Training on Device: {agent.device}")
    ...
    battery_stats = {i: {'picked_up': 0, 'picked_success': 0, 'picked_fail': 0, 'ignored_fail': 0, 'ignored_success': 0}
                     for i in range(len(MAP_FILES))}
    obstacle_stats = {i: {'avoided_won': 0, 'avoided_died': 0, 'hit_died': 0, 'hit_won': 0} for i in
                      range(len(MAP_FILES))}
    ...
    for episode in range(start_episode, NUM_EPISODES):
        ...
        for step in range(MAX_STEPS):
            ...
            next_state = get_full_state(current_observation, submarine, map_idx, cave_env.environment_width,
                                        cave_env.batteries)
    ...
    # Save Loss
    np.save("ddqn_training_loss.npy", np.array(loss_history))
    ...
    for i, filename in enumerate(MAP_FILES):
        ...
        print(
            f"{filename.split('/')[-1]:<40} | {goals:<5} | {attempts:<8} | {success_rate:<11.1f}% | {avg_reward:<10.1f}")
    ...
    for i, filename in enumerate(MAP_FILES):
        if map_stats[i]['attempts'] > 0:
            ...
            print(
                f"{display_name:<25} | {cs['clean_win']:<11} | {cs['dirty_win']:<11} | {cs['clean_fail']:<11} | {cs['dirty_fail']:<11}")
