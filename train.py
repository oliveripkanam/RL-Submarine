...
from src.ai.agent import VanillaDQNAgent

# Configuration
WATCH_MODE = False
LOAD_MODEL = False        # CHANGED: False for fresh comparison
NUM_EPISODES = 50000
MAX_STEPS = 4000
BATCH_SIZE = 128
EPSILON_START = 1.0       # CHANGED: 1.0 for fresh start
EPSILON_END = 0.01
EPSILON_DECAY = 0.99995   # CHANGED: Slower decay to match get_epsilon math
TARGET_UPDATE = 1000
SAVE_INTERVAL = 50
...
# --- HELPER: Smart Epsilon Decay (Copied from Code 3) ---
...
pygame.display.set_caption("Vanilla DQN Training")
...
# --- UPDATED INPUTS (Matches Code 3 Baseline) ---
...
def train():
    ...
    if not LOAD_MODEL:
        files = glob.glob("models/vanilla_dqn_*.pth")
        for f in files:
            try: os.remove(f)
            except: pass
        print("Cleared previous Vanilla models.")
    ...
    # Initialize Vanilla Agent
    agent = VanillaDQNAgent(input_shape=39, num_actions=5)
    
    total_steps = 0
    print(f"Starting Vanilla DQN Training on Device: {agent.device}")
    ...
    for episode in range(start_episode, NUM_EPISODES):
        # --- UPDATED: Use Smart Epsilon Function ---
        epsilon = get_epsilon(episode)

        # --- UPDATED: "Hard Mode" Curriculum (Matches Code 3) ---
    ...
    np.save("vanilla_training_loss.npy", np.array(loss_history))
